        ...
        var A; //Area in m^2
        var forceamp;//Amplitude of external force
        var forcefreq;//Frequency of external force
        ...
        function varinit()
        {
            ...
            $('#forceslider').slider("value",900);
            $('#frequencyslider').slider("value",105.2);
            $('#forcespinner').spinner("value",900);
            $('#frequencyspinner').spinner("value",105.2);
        }
        function varchange()
        {
            $('#forcespinner').spinner({ max : 1000, min : 100, step : 1 });
            $('#forceslider').slider({ max : 1000, min : 100, step : 1 });
        ...
        function plotgraph()
        {
            ...
            ctx.strokeStyle = "blue";
            for(let i =0;i<230;){        
                ctx.lineTo(i+20,125-5*actdisplace(.003*i)/5);
                ctx.moveTo(i+20,125-5*actdisplace(.003*i)/5);
                i+=.01;
                }
            ctx.stroke();
